...
head_rules = [
    ...
    "    # ── Telegram FakeTLS MTProxy probes: the macOS Swift client dials TG's ──",
    "    # ── own proxy pool (Vodafone-UK hosting, list fetched via DoH apv3.stel.com) ──",
    "    # ── on 80/443/5222 with a google SNI (bugs.telegram.org/c/36949 — by design). ──",
    "    # ── Via the airport these time out and retry forever => log flood. REJECT ──",
    "    # ── stops the noise; real DC-direct (149.154.x/91.108.x) is unaffected. ──",
    "    # ── TG rotates this pool — if a new dead IP starts flooding, add it here. ──",
    "    - 'IP-CIDR,194.221.250.50/32,REJECT,no-resolve'",
